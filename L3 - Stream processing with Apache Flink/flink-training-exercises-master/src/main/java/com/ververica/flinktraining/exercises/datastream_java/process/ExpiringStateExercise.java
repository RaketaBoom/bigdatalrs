...
public class ExpiringStateExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends KeyedCoProcessFunction<Long, TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		private static final long TIMEOUT_MS = 60000; // 1 минута
		...
		public void processElement1(TaxiRide ride, Context ctx, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			if (fare != null) {
				// Уже есть ожидающий fare – спариваем
				fareState.clear();
				deleteTimer(ctx);
				out.collect(Tuple2.of(ride, fare));
			} else {
				// Сохраняем ride и устанавливаем таймер
				rideState.update(ride);
				long timer = ctx.timestamp() + TIMEOUT_MS;
		...
		public void processElement2(TaxiFare fare, Context ctx, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			if (ride != null) {
				// Уже есть ожидающий ride – спариваем
				rideState.clear();
				deleteTimer(ctx);
				out.collect(Tuple2.of(ride, fare));
			} else {
				// Сохраняем fare и устанавливаем таймер
		...
		public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			// По таймеру отправляем неспаренный элемент в side output
			if (rideState.value() != null) {
				ctx.output(ExpiringStateExercise.unmatchedRides, rideState.value());
				rideState.clear();
			} else if (fareState.value() != null) {
				ctx.output(ExpiringStateExercise.unmatchedFares, fareState.value());
				fareState.clear();
			}
			timerState.clear();
		}
